...
- `channel-gateway` is the single lifecycle path for configured and dynamic UI attachment. It
  ...
  prune themselves. It bounds every identifier, path, prompt and metadata field before JSON parsing,
  fingerprinting, lane admission or imported calls. Only explicit resume unavailability falls back
...
- `sub-agent-host` composes through the generated `agent-host` import, durably journals both
  ...
  [spawn concurrency](docs/sub-agent-spawn-concurrency.png) and
