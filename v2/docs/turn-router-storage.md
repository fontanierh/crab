...
- Queue and steer map directly to native channel input. Interrupt first performs the explicit
  channel cancel action, tolerates an already-idle session, then submits the trigger as queue input.
