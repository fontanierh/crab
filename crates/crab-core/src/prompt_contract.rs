...
fn render_messaging_semantics_section() -> String {
    "## MESSAGING_SEMANTICS\n- Normal assistant text is delivered to Discord by Crab.\n- Crab may stream output by editing the active message while you generate text.\n- Separate each logical section of your response with a blank line (`\\n\\n`). Crab uses blank lines as message boundaries — each section becomes its own Discord message.\n- Keep individual sections concise (a few sentences). Do not write one long unbroken wall of text.\n- Use Discord messaging tools only for explicit Discord actions (send/edit/delete/react/moderation/proactive operations).\n- Do not emit duplicate plain-text confirmations for actions already completed via Discord tools."
        .to_string()
}
