    ...
    RB: FnOnce(OwnerConfig, D) -> CrabResult<DaemonTurnRuntime<D>>,
{
    ...
    loop {
        ...
        // Batch by channel so multiple triggers become one combined run.
        {
            ...
            let steering_count = steering.len();
            ...
            stats.ingested_triggers = stats
                .ingested_triggers
                .saturating_add(steering_count as u64);

            let graceful = crab_core::read_graceful_steering_triggers(&state_root)?;
            let graceful_count = graceful.len();
            executor.consume_and_batch_triggers(
                &graceful,
                "", // no current lane when idle
            ...
            stats.ingested_triggers = stats
                .ingested_triggers
                .saturating_add(graceful_count as u64);
        }
